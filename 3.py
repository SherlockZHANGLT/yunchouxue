...
xinlist_test=np.load('xinlist_test.npy')  # 10*8测试初始车辆分布
...
# Model formulation
m = gb.Model('train')
A0 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='A0')  #时段1的运载数
A1 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='A1')
A2 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='A2')
A3 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='A3')
A4 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='A4')
A5 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='A5')

B0 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='B0')  #时段1的运载数
B1 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='B1')
B2 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='B2')
B3 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='B3')
B4 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='B4')
B5 = m.addVars(64*20, lb=0, vtype=GRB.INTEGER, name='B5')

a = m.addVars(64, lb=0, vtype=GRB.CONTINUOUS, name='a')  

b0 = m.addVars(64, lb=0, vtype=GRB.CONTINUOUS, name='b0')  
b1 = m.addVars(64, lb=0, vtype=GRB.CONTINUOUS, name='b1')
b2 = m.addVars(64, lb=0, vtype=GRB.CONTINUOUS, name='b2')
b3 = m.addVars(64, lb=0, vtype=GRB.CONTINUOUS, name='b3')
b4 = m.addVars(64, lb=0, vtype=GRB.CONTINUOUS, name='b4')
b5 = m.addVars(64, lb=0, vtype=GRB.CONTINUOUS, name='b5')

#B的取整
m.addConstrs((B0[i] <= a[i%64]+b0[i%64]*dlist_train[i//64][(i%64)//8][i%8][0] for i in range(64*20)), 'c1')
m.addConstrs((B1[i] <= a[i%64]+b1[i%64]*dlist_train[i//64][(i%64)//8][i%8][1] for i in range(64*20)), 'c2')
m.addConstrs((B2[i] <= a[i%64]+b2[i%64]*dlist_train[i//64][(i%64)//8][i%8][2] for i in range(64*20)), 'c3')
m.addConstrs((B3[i] <= a[i%64]+b3[i%64]*dlist_train[i//64][(i%64)//8][i%8][3] for i in range(64*20)), 'c4')
m.addConstrs((B4[i] <= a[i%64]+b4[i%64]*dlist_train[i//64][(i%64)//8][i%8][4] for i in range(64*20)), 'c5')
m.addConstrs((B5[i] <= a[i%64]+b5[i%64]*dlist_train[i//64][(i%64)//8][i%8][5] for i in range(64*20)), 'c6')
m.addConstrs((B0[i] >= a[i%64]+b0[i%64]*dlist_train[i//64][(i%64)//8][i%8][0]-1 for i in range(64*20)), 'c7')
m.addConstrs((B1[i] >= a[i%64]+b1[i%64]*dlist_train[i//64][(i%64)//8][i%8][1]-1 for i in range(64*20)), 'c8')
m.addConstrs((B2[i] >= a[i%64]+b2[i%64]*dlist_train[i//64][(i%64)//8][i%8][2]-1 for i in range(64*20)), 'c9')
m.addConstrs((B3[i] >= a[i%64]+b3[i%64]*dlist_train[i//64][(i%64)//8][i%8][3]-1 for i in range(64*20)), 'c10')
m.addConstrs((B4[i] >= a[i%64]+b4[i%64]*dlist_train[i//64][(i%64)//8][i%8][4]-1 for i in range(64*20)), 'c11')
m.addConstrs((B5[i] >= a[i%64]+b5[i%64]*dlist_train[i//64][(i%64)//8][i%8][5]-1 for i in range(64*20)), 'c12')
...
for v in m.getVars():
    ...
    elif(('a' in v.varName or 'b' in v.varName) and float(v.x)!=0.0):
        print('%s %g' % (v.varName, v.x))
...
for n in range(10): 
    ...
    #B的取整
    m.addConstrs((B0[i] <= a_p[i]+b0_p[i]*dlist_test[n][i//8][i%8][0] for i in range(64)), 'c1')
