...
a0 = m.addVars(64, vtype=GRB.CONTINUOUS, name='a0')
a1 = m.addVars(64, vtype=GRB.CONTINUOUS, name='a1')
a2 = m.addVars(64, vtype=GRB.CONTINUOUS, name='a2')
a3 = m.addVars(64, vtype=GRB.CONTINUOUS, name='a3')
a4 = m.addVars(64, vtype=GRB.CONTINUOUS, name='a4')
a5 = m.addVars(64, vtype=GRB.CONTINUOUS, name='a5')  
...
#B的取整
m.addConstrs((B0[i] == a0[i%64]+b0[i%64]*(dlist_train[i//64][(i%64)//8][i%8][0])+s[i%64]*(sum_train[i//64]-xinlist_train[i//64][(i%64)//8]) for i in range(64*20)), 'c1')
m.addConstrs((B1[i] == a1[i%64]+b1[i%64]*(dlist_train[i//64][(i%64)//8][i%8][1]) for i in range(64*20)), 'c2')
m.addConstrs((B2[i] == a2[i%64]+b2[i%64]*(dlist_train[i//64][(i%64)//8][i%8][2]) for i in range(64*20)), 'c3')
m.addConstrs((B3[i] == a3[i%64]+b3[i%64]*(dlist_train[i//64][(i%64)//8][i%8][3]) for i in range(64*20)), 'c4')
m.addConstrs((B4[i] == a4[i%64]+b4[i%64]*(dlist_train[i//64][(i%64)//8][i%8][4]) for i in range(64*20)), 'c5')
m.addConstrs((B5[i] == a5[i%64]+b5[i%64]*(dlist_train[i//64][(i%64)//8][i%8][5]) for i in range(64*20)), 'c6')
...
for v in m.getVars():
    ...
    #print('%s %g' % (v.varName, v.x))
print('trainning ending.............................................................')

print(m.objVal)
for v in m.getVars():
    if('a0' in v.varName):
        a0_p.append(float(v.x))
    elif('a1' in v.varName):
        a1_p.append(float(v.x))
    elif('a2' in v.varName):
        a2_p.append(float(v.x))
    elif('a3' in v.varName):
        a3_p.append(float(v.x))
    elif('a4' in v.varName):
        a4_p.append(float(v.x))
    elif('a5' in v.varName):
        a5_p.append(float(v.x))
...
for n in range(10): 
    ...
    #B的取整
    m.addConstrs((B0[i] <= a0_p[i]+b0_p[i]*(dlist_test[n][i//8][i%8][0])+s_p[i]*(sum_test[n]-xinlist_test[n][i//8])+0.5 for i in range(64)), 'c1')
    m.addConstrs((B1[i] <= a1_p[i]+b1_p[i]*(dlist_test[n][i//8][i%8][1])+0.5 for i in range(64)), 'c2')
    m.addConstrs((B2[i] <= a2_p[i]+b2_p[i]*(dlist_test[n][i//8][i%8][2])+0.5 for i in range(64)), 'c3')
    m.addConstrs((B3[i] <= a3_p[i]+b3_p[i]*(dlist_test[n][i//8][i%8][3])+0.5 for i in range(64)), 'c4')
    m.addConstrs((B4[i] <= a4_p[i]+b4_p[i]*(dlist_test[n][i//8][i%8][4])+0.5 for i in range(64)), 'c5')
    m.addConstrs((B5[i] <= a5_p[i]+b5_p[i]*(dlist_test[n][i//8][i%8][5])+0.5 for i in range(64)), 'c6')
    m.addConstrs((B0[i] >= a0_p[i]+b0_p[i]*(dlist_test[n][i//8][i%8][0])+s_p[i]*(sum_test[n]-xinlist_test[n][i//8])-0.5+1e-4 for i in range(64)), 'c7')
    m.addConstrs((B1[i] >= a1_p[i]+b1_p[i]*(dlist_test[n][i//8][i%8][1])-0.5+1e-4 for i in range(64)), 'c8')
    m.addConstrs((B2[i] >= a2_p[i]+b2_p[i]*(dlist_test[n][i//8][i%8][2])-0.5+1e-4 for i in range(64)), 'c9')
    m.addConstrs((B3[i] >= a3_p[i]+b3_p[i]*(dlist_test[n][i//8][i%8][3])-0.5+1e-4 for i in range(64)), 'c10')
    m.addConstrs((B4[i] >= a4_p[i]+b4_p[i]*(dlist_test[n][i//8][i%8][4])-0.5+1e-4 for i in range(64)), 'c11')
    m.addConstrs((B5[i] >= a5_p[i]+b5_p[i]*(dlist_test[n][i//8][i%8][5])-0.5+1e-4 for i in range(64)), 'c12')
    ...
    for v in m.getVars():
        ...
        if('B' in v.varName):
            if(int(v.x)!=0):
                print('%s %g' % (v.varName, v.x))
                if(dlist_test[n][(temp%64)//8][temp%8][1+temp//64]-A_p[temp] !=0):
                    if(dlist_test[n][(temp%64)//8][temp%8][1+temp//64]-A_p[temp]>=int(v.x)):
                        sum+=int(v.x)*(baselen[(temp%64)//8][temp%8]*(dis_income+empty_cost)+IDL[(temp%64)//8][temp%8]*time_income)
                    else:
                        sum+=(dlist_test[n][(temp%64)//8][temp%8][1+temp//64]-A_p[temp])*(baselen[(temp%64)//8][temp%8]*(dis_income+empty_cost)+IDL[(temp%64)//8][temp%8]*time_income)
            temp=temp+1
        #print('%s %g' % (v.varName, v.x))
    sum+=float(m.objVal)
    print('ending.............................................................')
